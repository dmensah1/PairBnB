...
export class PlacesService {
  ...
  fetchPlaces() {
    ...
    .get<{ [key: string]: PlaceData }>(`https://pairbnb-app.firebaseio.com/offered-places.json?auth=${token}`)
    }),
    map(
      resData => {
  ...
  getPlace(id: string) {
    ...
    return this.http
    .get<PlaceData>(
      `https://pairbnb-app.firebaseio.com/offered-places/${id}.json?auth=${token}`
    )
    }),
      map(placeData => {
  ...
  addPlace(title: string, description: string, price: number, dateFrom: Date, dateTo: Date, location: PlaceLocation, imageUrl: string) {
    ...
    newPlace = new Place(
      ...
      // making the post request to server as an obserable
       return this.http
       .post<{name: string}>(`https://pairbnb-app.firebaseio.com/offered-places.json?auth=${token}`,{...newPlace, id: null })
    }), 
    switchMap(resData => {
  ...
  updatePlace(placeId: string, title: string, description: string) {
    ...
    return this.authService.token.pipe(
      ...
      switchMap(places => {
        ...
        `https://pairbnb-app.firebaseio.com/offered-places/${placeId}.json?auth=${fetchedToken}`,
